...
namespace Assignment5
{
    ...
    {Не знайшла реалізації для файлів алгоритмом злиття, а також пірамідальним сортуванням
        ...
        #endregion
        #region MergeSortRegion
        public void Merge( int lb, int mid, int ub)
        {
            int lowPartInd = lb;
            int newArrInd = 0;
            int upperPartInd = mid + 1;
            int[] newArr = new int[ub - lb + 1];
            while (lowPartInd <= mid && upperPartInd <= ub)
            {
                if (arr[lowPartInd] <= arr[upperPartInd])
                {
                    newArr[newArrInd] = arr[lowPartInd];
                    ++lowPartInd;
                }
                else
                {
                    newArr[newArrInd] = arr[upperPartInd];
                    ++upperPartInd;
                }
                ++newArrInd;
            }
            if (lowPartInd > mid)
            {
                while (upperPartInd <= ub)
                {
                    newArr[newArrInd] = arr[upperPartInd];
                    ++upperPartInd; ++newArrInd;
                }
            }
            else
            {
                while (lowPartInd <= mid)
                {
                    newArr[newArrInd] = arr[lowPartInd];
                    ++lowPartInd; ++newArrInd;
                }
            }
